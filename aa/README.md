...
wrapper the widget hides for its other mode hides its blocks too. The packer measures each block's natural
width once, then tries 1–4 columns in two variants (a block wider than a column becomes a full-width band at
its own width, or the zoom is capped so it fits a column), splits the remaining blocks into columns of nearly
equal height with the linear-partition DP (block order kept), and scales the whole widget with CSS `zoom`
(0.62–1.7) so the tallest column just fills the stage. The variant with the largest zoom wins; leftover space
under a column goes to a scroll box in it, and a listing that cannot fit even at the smallest zoom scrolls.
Text is one size everywhere in the widget. During playback only the row spans and the zoom follow the frames;
the columns stay put until a click, a block appearing or vanishing, or a resize triggers a new packing.
...
frames at 1366×768 and 1920×1080 (expanded), inline at 1366 px and at 390 px: no console errors, no scrolling
stage, no clipped code, no overlapping blocks.
