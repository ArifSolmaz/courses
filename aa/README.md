...
(previous/next, chooser, zoom, Expand), a stage bounded to the viewport, and a hint line. `assets/compact.js`
groups every widget at display time into a left pane (the code with its variables and output side by side)
and a right pane (the test/predict rows, the visual, stats, message); from 860 px of stage width they sit
side by side, each pane sized to the space under the title and above the play bar and scaled down (never
below 72 %) when its natural height exceeds it; widgets without a code block lay their pieces out in a
two-column dense grid instead; Expand moves the same nodes into a full-screen dialog. Widgets keep their own DOM, so `anim-w*.js`
and `anim-aa.js` need no changes; the only rule is that a widget's play bar is a direct child `.anim-controls`
(the shared `Player` does this). Layout rules are in `assets/course.css` under "animation workspace".
