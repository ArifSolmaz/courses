...
groups every widget at display time into a left pane (code, variables/output, stats, message) and a right pane
(the visual); from 860 px of stage width they sit side by side and are sized so the play bar is always on
screen; Expand moves the same nodes into a full-screen dialog. Widgets keep their own DOM, so `anim-w*.js`
