...
WEEK_NOTES = {
1:('A robot route needs a counterexample',[1],
'''<p>A robot visits solder points on a circuit board and returns to its start. “Choose the nearest unvisited point” sounds sensible, but a local saving can create an expensive later move. Before writing code, specify the legal inputs, the distance being measured and whether the route must return home.</p><p>On a line, any closed route visiting both extremes travels at least twice their separation. That gives an independent lower bound. A sweep from one extreme to the other and back can reach it. Compare the nearest-neighbour trace in the investigation with that bound. One counterexample disproves an “always optimal” claim; many successful tests do not prove it.</p>''',
'If an AI proposes a routing rule, ask for its objective, tie rule and a small case that defeats it.',
...
'''<p>An algorithm solves a family of inputs. A program is one implementation. Sorting means returning the same items in nondecreasing order; it does not mean printing a particular example. Write names for the input, its size and the output before you choose Python statements.</p><p>For sensor records, specify what is being ordered: timestamp, temperature or sensor identifier. Repeated readings are still separate records. Printing a plausible list is not enough: the output must preserve every record as well as satisfy the ordering rule.</p><p>The RAM model is a later tool for counting simple operations. For now, notice that <code>x = a + b</code> performs a fixed amount of work on fixed-size values, while an instruction such as “sort all readings” hides work that grows with the data.</p>''',
'Ask a teammate to check your output specification without seeing your code.',
'A three-line input/output contract and a tiny example containing duplicate values.'),
...
'For a collision check between parts, distinguish all ordered pairs, unique pairs, and pairs including a part with itself.',
'A hand-drawn trace at n = 4 and the operation being counted.'),
4:('The order of service changes the system',[4],
'''<p>A list stores items, but a stack or queue also specifies which item leaves next. A stack removes the most recently added item; a queue removes the earliest. These are behavioural rules, independent of the implementation.</p><p>Imagine three inspection jobs arriving as 1, 2 and 3. A last-in-first-out stack returns 3 then 2. A first-in-first-out queue returns 1 then 2. The same stored values produce different behaviour because the service rule changed.</p><p>A Python list gives direct access to a position. A linked list reaches a position by following links. Both can grow, but they pay different costs for access and rearrangement. Do not assume that a container's name proves it is appropriate for the job.</p>''',
...
'''<p>The notes separate best, worst and average cost over inputs of the same size. A timing measurement is one observation on one machine, implementation and input. It does not automatically measure any of those three functions.</p><p>Searching a hundred readings can stop at the first reading, stop at the last, or exhaust the list without finding the target. Keep n fixed and compare those cases. Repeat measurements and record what setup work you included. The worst observed time is evidence about your trials, not proof of the worst possible time.</p><p>A theoretical operation bound complements the stopwatch. It helps explain why the work changes. A real controller deadline additionally depends on execution environment, memory behaviour and other tasks.</p>''',
'Write “largest time in these trials” rather than “guaranteed maximum” unless you have established that guarantee.',
'A timing table with input cases, repeat count, units and one limitation.'),
6:('An extrapolation is a model, not a promise',[2,3],
'''<p>If time is approximately proportional to n², doubling n predicts four times the time. If it is proportional to n, doubling predicts twice the time. These are model predictions to test against fresh observations.</p><p>Suppose 1,000 records take one second. At 10,000 records, a quadratic model predicts 100 seconds. An n log₂ n model predicts about 13.3 seconds. The same starting measurement supports very different forecasts until you identify the growth pattern.</p><p>A Big-Theta class alone does not determine a finite-size timing ratio. These estimates assume a stable leading constant and negligible lower-order effects. Record the residual—the difference between prediction and observation—and investigate when the model stops fitting.</p>''',
...
'''<p>To find the maximum of n distinct readings using comparisons, every reading except the winner must lose at least one comparison. A comparison can create only one new loser. Therefore at least n − 1 comparisons are necessary in the worst case.</p><p>A single scan achieves n − 1 comparisons, so its comparison count meets the lower bound. This is stronger than saying “my code looks short”: it explains why a different comparison-based method cannot remove that work.</p><p>State your model. The argument counts comparisons on distinct values; it does not count every instruction or memory access. Correctness and cost proofs each need assumptions, a base case and a reason the next step preserves the claim.</p>''',
...
8:('A bound needs a constant and a threshold',[2,3],
'''<p>Big-O is an eventual upper bound. To show 2n² + 6n is O(n²), choose c = 3 and n₀ = 6. For n ≥ 6, the linear term 6n is at most n², so the whole expression is at most 3n².</p><p>Big-Omega is an eventual lower bound; Big-Theta means both bounds hold. A quadratic function is also O(n³), but that upper bound is loose. Naming a bound is different from selecting a best, average or worst input case: apply the notation to whichever cost function you have defined.</p><p>Test your reasoning with a claim that fails. If n² were O(n), then n² ≤ cn would imply n ≤ c for every sufficiently large n. No fixed c can satisfy that.</p>''',
...
'''<p>The notes use sorting and hashing to turn repeated searching into organised work. To find two records whose values sum to T, one strategy sorts then moves pointers inward; another remembers earlier values in a hash set and looks for T − x.</p><p>Preparation matters. Sorting is part of the first method's total cost. Hashing needs extra storage, a suitable distribution and a rule that prevents using one record twice. Check the complement before inserting the current record, or track counts explicitly.</p><p>This is the same habit as comparing anagram methods: state the output contract, trace a small case, count the expensive operations, and include setup. An expected fast method and a worst-case guarantee are different promises.</p>''',
...
'''<p>A dynamic array keeps spare capacity. In the simplified doubling model, growth copies 1, 2, 4, … items into successively larger arrays. To reach n = 2ᵏ stored items from capacity 1, the copies total n − 1, even though the final resize alone is expensive.</p><p>This gives constant amortised copying cost per append over a sequence. It does not make every individual append constant-time. Growing by only ten slots instead repeatedly copies nearly the whole prefix, producing quadratic total copying.</p><p>Python's exact allocation policy is an implementation detail; the doubling model explains the idea rather than specifying its precise behaviour. Keep “amortised over a sequence” separate from “expected over random choices” and “worst case for one operation”.</p>''',
'A logging task may tolerate amortised cost while a control-loop deadline may be sensitive to the single resize.',
...
'''<p>A hash function maps keys to slots. Different keys can share a slot, so correctness depends on collision handling. With h(k) = k mod 10, keys 12, 22, 32 and 42 all enter slot 2. Changing the modulus to 11 separates this set, but does not guarantee separation for every set.</p><p>Chaining keeps several entries per slot. Open addressing searches a sequence of slots. In open addressing, simply clearing a deleted slot can make later keys appear absent; a deleted marker preserves the search path.</p><p>Expected constant-time lookup needs suitable hashing and controlled load. The worst case can still be linear. For a deadline-sensitive lookup, compare a hashing design with one offering a more predictable bound.</p>''',
...
'''<p>Binary search repeatedly discards half the candidate positions. Its logarithmic search cost relies on efficient access to the middle position. A sorted array supports that access; a linked list has to walk through links to reach the middle.</p><p>With one candidate, a search may still need a comparison. For a conventional successful binary search on n ≥ 1 array items, the worst comparison count is floor(log₂ n) + 1. “About log n” is useful for growth, but an exact count needs an implementation and a convention.</p><p>A balanced search tree offers a different route: follow one link per level, keeping height logarithmic. An ordinary unbalanced tree can become a chain. Sorted data, random insertion order and guaranteed balance are not interchangeable conditions.</p>''',
'Choose between an array of calibration points and a linked record chain by examining the actual access operations.',
'A search trace and a list of the prerequisites your claimed bound needs.'),
13:('Merge first, then explain the full sort',[3,7,8,9],
'''<p>Merging two sorted lists only compares their first unconsumed items. Each comparison commits one item to the output. When one list empties, copy the rest without more key comparisons. With n items in total, at most n − 1 comparisons are needed when both lists start nonempty.</p><p>Mergesort repeats this linear merge across logarithmically many levels. Quicksort partitions instead: random pivots give expected n log n work under appropriate assumptions, but a poor partition sequence can be quadratic. Equal keys require care; three-way partitioning handles them explicitly.</p><p>The comparison-sorting lower bound does not forbid every linear-time sort. Counting or radix methods exploit restricted key representations. Check the key range, stability requirements and memory cost before claiming a faster solution.</p>''',
...
14:('Defend a design with an exchange argument',[1,7,19],
'''<p>Suppose 2n task durations must be paired onto n work stations and the objective is to minimise the largest pair total. Sort the durations and pair the smallest with the largest. A proof can explain why the rule works.</p><p>Take an optimal pairing where the smallest a is paired with x and the largest b with y. Replace those pairs by (a,b) and (x,y). Since a ≤ y and x ≤ b, neither new sum exceeds the old sum b + y. The maximum cannot increase. Repeat on the remaining durations.</p><p>The objective is crucial. This proof covers fixed pairs and that maximum-sum objective; it does not solve every scheduling problem. When the task changes, reconsider the model before reusing the rule. The optional extension introduces networks, dynamic programming and the limits of exact methods.</p>''',
...
'A one-page decision note with a trace, cost model, test evidence and a limitation.'),
}
...
def weekly_material(num):
    ...
    focus=f'''<section class="source-focus" id="skiena-practice"><h3>One problem to investigate</h3><p>Sketch or trace your answer before revealing the reasoning. Use this within the class workshop; it is not an extra assignment.</p>{exercise(ids[0])}<p><strong>Keep as evidence:</strong> {evidence}</p><p lang="tr" class="source-hint">Önce tahmin et, küçük bir örneği elle izle, sonra çözümle karşılaştır. Varsayımını ve bulduğun kanıtı açıkla.</p></section>'''
    more=''
    if len(ids)>1:
        more='<details class="path-reference source-bank" id="skiena-more"><summary>Optional practice from the supplied notes</summary><div><p>Choose a question for review, rather than completing the whole list. Some extend beyond this week’s core; use the later lessons when prerequisites are unfamiliar. Exercise numbers match the source document.</p>'+''.join(exercise_choice(n) for n in ids[1:])+'</div></details>'
    if num==14:
