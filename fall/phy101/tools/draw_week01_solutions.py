...
def arrow(ax,start,end,label,color=BLUE,offset=(7,7),style='-'):
    ax.annotate('',xy=end,xytext=start,arrowprops={'arrowstyle':'-|>','color':color,'lw':2.3,'linestyle':style,'mutation_scale':15})
    if label:ax.annotate(label,xy=(np.array(start)+end)/2,xytext=offset,textcoords='offset points',color=color,fontsize=12)
...
def flow(n,title,steps,footer):
    ...
    for j,(y,txt) in enumerate(zip(ys,steps)):
        ...
        if j and n != 14:ax.annotate('',xy=(.5,y+.065),xytext=(.5,ys[j-1]-.065),arrowprops=dict(arrowstyle='->',color=GRAY))
...
for n,title,edge,note in [(2,'1.2 | Convert all three edges','1 in = 2.54 cm',r'$1\ \mathrm{in^3}=(2.54)^3\ \mathrm{cm^3}$'),(4,'1.4 | The volume unit is in the denominator','1 m = 100 cm',r'$1\ \mathrm{m^3}=10^6\ \mathrm{cm^3}$')]:
    ...
    square=np.array([[0,0],[2,0],[2,2],[0,2],[0,0]]);off=np.array([.8,.65]);ax.plot(*square.T,color=BLUE);ax.plot(*(square+off).T,color=BLUE)
    for p in square[:4]:ax.plot([p[0],p[0]+off[0]],[p[1],p[1]+off[1]],color=BLUE)
    ax.text(1,-.28,edge,ha='center');ax.text(3.4,1.3,'same\nedge',ha='center',color=GRAY);ax.text(1.4,3.05,note,ha='center',fontsize=15);save(fig,n)
flow(3,'1.3 | Light moving through vacuum',['Start: t = 0','distance d = 1.00 ft = 0.3048 m','Finish: t = d / c'],'The travel time is a duration, not a clock reading.')
...
flow(14,'1.14 | Compare after rounding to six significant figures',['π = 3.141592653… → 3.14159','22 / 7 = 3.142857142… → 3.14286','355 / 113 = 3.141592920… → 3.14159'],'Only the second rational approximation agrees at six significant figures.')
...
for n,v,start,end,lab in [(26,(-16,-16/np.tan(np.deg2rad(34))),236,270,'34° from −y, clockwise'),(27,(-9.6*np.tan(np.deg2rad(32)),9.6),90,122,'32° from +y, counter-clockwise')]:
 fig,axs=panels([f'1.{n} | {lab}']);ax=axs[0];axes(ax,[v],'m');vector(ax,v,'A',components=True);r=np.linalg.norm(v)*.26;ax.add_patch(Arc((0,0),2*r,2*r,theta1=start,theta2=end,color=ORANGE,lw=2));ax.text(.04,.96,f'x < 0; y {"<" if n==26 else ">"} 0',transform=ax.transAxes,va='top',fontsize=11);save(fig,n)
...
fig,axs=panels(['1.32 | A + B must reach the given endpoint']);ax=axs[0];axes(ax,[A,R,B],'m');vector(ax,A,'A');arrow(ax,A,R,'B',ORANGE,(-20,0));vector(ax,R,'R',GREEN,(7,0));vector(ax,B,'B',ORANGE,(8,0));save(fig,32)
...
for ax,u,v,lu,lv in zip(axs,[A,A,B],[B,-B,-A],['A','A','B'],['B','−B','−A']):axes(ax,[u,u+v],'cm');vector(ax,u,lu);arrow(ax,u,u+v,lv,ORANGE);vector(ax,u+v,'result',GREEN,(-55,10))
save(fig,33)
for n in [36,40,42]:
 A=np.array([4.,7.]);B=np.array([5.,-2.]);fig,axs=panels([f'1.{n} | Vectors with a common origin']);ax=axs[0];axes(ax,[A,B,A-B]);vector(ax,A,'A');vector(ax,B,'B',ORANGE)
 if n==36:arrow(ax,B,A,'A − B',GREEN,(8,0));vector(ax,A-B,'A − B',GREEN,(8,0))
  ...
  if n==42:ax.text(.04,.96,'A → B: clockwise\nA × B: into page (−z)',transform=ax.transAxes,va='top',fontsize=11)
 save(fig,n)
fig=plt.figure(figsize=(7,6),layout='constrained');ax=fig.add_subplot(projection='3d');ax.set_title('1.39 | Three-dimensional vectors (perspective view)')
for v,l,c in [((-2,3,4),'A',BLUE),((3,1,-3),'B',ORANGE),((-5,2,7),'A − B',GREEN)]:ax.quiver(0,0,0,*v,color=c,arrow_length_ratio=.1,linewidth=2);ax.text(*v,l,color=c)
ax.set_xlim(-6,4);ax.set_ylim(-4,6);ax.set_zlim(-3,7);ax.set_box_aspect((1,1,1));ax.set_xlabel('x');ax.set_ylabel('y');ax.set_zlabel('z');save(fig,39)
fig,axs=panels(['1.43(a) | Expect an obtuse angle','1.43(b) | Expect an acute angle','1.43(c) | Test for perpendicular vectors'])
for ax,A,B in zip(axs,[(-2,6),(3,5),(-4,2)],[(2,-3),(10,6),(7,14)]):axes(ax,[A,B]);vector(ax,A,'A');vector(ax,B,'B',ORANGE)
save(fig,43)
A=np.array([1.4,2.8*np.sin(np.pi/3)]);B=np.array([.95,-1.9*np.sin(np.pi/3)])
fig,axs=panels(['1.44 | The parallelogram and its normal']);ax=axs[0];axes(ax,[A,B,A+B],'cm');ax.add_patch(Polygon([(0,0),A,A+B,B],fc='#e5f2eb',ec=GREEN,alpha=.7));vector(ax,A,'A');vector(ax,B,'B',ORANGE);ax.add_patch(Arc((0,0),1.3,1.3,theta1=-60,theta2=60,color=GREEN,lw=2));ax.text(.72,0,'120°',color=GREEN);ax.text(.04,.97,'A × B: into page (−z)\nB × A: out of page (+z)',transform=ax.transAxes,va='top',fontsize=11);save(fig,44)
