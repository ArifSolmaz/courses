...
Reviewed 11 September 2026 against the local course notes, the supplied lecture/laboratory schedule image, and the separate `phy101-solutions` repository.

## Screenshot-driven notebook revision
...
**Final verification:** all **27 course notebooks** pass schema and fresh-kernel execution checks: **133 source code cells and 37 dated-lesson code cells**. The tested code hashes match the final files. All 140 problem identifiers and 42 source checkpoints are present. Calendar assembly reproduces, all calendar/release labels agree, and refreshing the shared interface is idempotent.

Live JupyterLab checks covered repeated vector updates, the reported 130° component diagram, maximum parallel addition, projection views, rendered equations/determinants/problem answers, the projectile table, and all four lesson navigation links. Desktop (1366 × 900) and narrow (480 × 800) panels were inspected; the narrow controls have no horizontal overflow. Independent plot reviews also exercised representative boundary values and animation frames. These checks sample parameter combinations; hosted Colab was not directly tested.

See [the final execution and browser-check record](notebook_review_verification.json). The complete textbook-format edition is prepared for the course website and Colab links. The private solutions were not modified in this follow-up. The corrected source answer keys in modules 06–07 agree with their existing private hand-worked solutions.

## Complete textbook-format pass
...
All 976 source/calendar Markdown cells were exported for a complete browser typesetting check. MathJax rendered 5,602 expressions across all 27 notebooks with no parse-error nodes. The final code hashes still match the successful execution record.
...
## Adopted calendar — implementation update, 11 September 2026
...
**Calendar QA completed:** all 13 weekly notebooks validate and reproduce from the builder. All 37 code cells in the 11 interactive weekly lessons pass fresh-kernel execution; their tested code matches the final files. Review and midterm are paper-only. The dashboard's 13 primary links return the matching notebook files; dashboard and syllabus show the same dates, topics and laboratories. All 14 release dates agree with the private publisher, whose output matches the expected modules at all 18 before/on-release boundaries. No notebooks were published during these checks.

The weekly selection includes 50 references to source problems, plus worked answers for every new recap, review and demonstration prompt. Independent content review checked the energy/rotation split, Newton introduction and early pendulum preparation. All 41 notebooks across the calendar, source library and private solutions pass schema validation.
...
## Verification of the enriched source edition

These additional checks cover the complete source library and solution collection. Calendar assembly preserves their numerical code; its validation is recorded above.

- All **28 notebooks** pass notebook-schema validation with unique cell IDs.
- All **14 source-module notebooks** execute from start to finish in fresh Python kernels. Their default interactive callbacks report no captured errors; final code hashes match the tested source.
...
Execution validates code and default parameter choices; it does not prove every possible slider combination or every physical assumption. Hosted Google Colab was not directly tested. See [maintenance instructions](tools/README.md) for refreshing embedded controls and rerunning the notebook checks.
