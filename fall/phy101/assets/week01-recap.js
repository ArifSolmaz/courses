...
function makeLab(host){
 ...
 host.innerHTML=`<h3>${is3?'3D edge and normal explorer':'Dot, cross and projection explorer'}</h3><p>${is3?'Blue a · rust b · green unit-normal direction. The drawing is a 2D projection. Choose one case, then explain the shape before showing values.':'Blue A · rust B. Choose one case at a time. Watch the height and the projection; show values only after making a prediction.'}</p><div class="recap-scene"></div><div class="recap-readouts" aria-live="off"></div><div class="recap-inputs"><label>Rotation of original second vector <output data-angle-output></output><input type="range" data-angle min="-180" max="180" step="0.1" aria-label="${key} vector angle"></label><label>Original second length (${m.unit}) <output data-length-output></output><input type="range" data-length min="0" max="${is3?4:8}" step="0.01" aria-label="${key} second edge length"></label>${is3?'<label>View rotation <output data-view-output></output><input type="range" data-view min="0" max="360" step="1" value="330" aria-label="3D view rotation"></label><label>Shared scale error <output data-scale-output></output><input type="range" data-scale min="-1" max="1" step="0.1" value="0" aria-label="Shared scale error percent"></label>':''}</div><div class="recap-presets" aria-label="${key} edge cases"><button data-case="original">Given vectors</button><button data-case="parallel">Parallel 0°</button><button data-case="perpendicular">Perpendicular 90°</button><button data-case="opposite">Opposite 180°</button><button data-case="clockwise">${is3?'Negative rotation −90°':'Clockwise −90°'}</button><button data-case="equal">Equal vectors</button>${is3?'<button data-case="near">Nearly parallel 1°</button>':''}<button data-case="zeroB">Zero second edge</button><button data-case="zeroA">Zero first edge</button><button data-case="swap">Swap order</button></div><div class="recap-play"><button type="button" data-play>Play angle sweep</button><button type="button" data-reset>Reset animation</button><span>No autoplay · 20 seconds per full sweep · drawing auto-fits</span></div><p class="recap-case-note"></p>`;
 const workspace=document.createElement('div');workspace.className='recap-workspace';
 const visual=document.createElement('div');visual.className='recap-visual';
 const controls=document.createElement('div');controls.className='recap-controls';
 for(const selector of ['.recap-scene','.recap-readouts'])visual.append(host.querySelector(selector));
 for(const selector of ['.recap-inputs','.recap-presets','.recap-play','.recap-case-note'])controls.append(host.querySelector(selector));
 const values=document.createElement('details');values.className='optional-values';values.innerHTML='<summary>Show numerical values</summary>';values.append(visual.querySelector('.recap-readouts'));visual.append(values);
 const advanced=document.createElement('details');advanced.className='optional-controls';advanced.innerHTML='<summary>Explore other values</summary>';advanced.append(controls.querySelector('.recap-inputs'));controls.append(advanced);
 controls.querySelector('.recap-play').remove();
 workspace.append(visual,controls);host.append(workspace);
 host.querySelectorAll('button').forEach(b=>b.type='button');
 const angle=host.querySelector('[data-angle]'),length=host.querySelector('[data-length]'),view=host.querySelector('[data-view]'),scale=host.querySelector('[data-scale]');
 ...
 function draw(){
  ...
  text(p.a,(is3?'a':'A')+(p.A<1e-10?' = 0':''),10,-14,colors.a);text(p.b,(is3?'b':'B')+(p.B<1e-10?' = 0':''),p.B<1e-10?12:-22,26,colors.b);
  ...
  svgEl('text',{x:18,y:384,'font-size':16,fill:'#465569'},g,is3?(p.n?'Normal arrow: direction only; drawn at a fixed length.':'Zero area: no unique normal direction.'):(p.C<1e-10?'Zero area · no cross-product direction':p.c[2]>0?'⊙  Cross product points out of the board (+z)':'⊗  Cross product points into the board (−z)'));
  ...
  if(s.swap)note+=p.n?' The order has been swapped: cross-product direction reverses, dot product and area do not.':' The order has been swapped; the zero cross product stays zero.';
