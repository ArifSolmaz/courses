...
(() => {
  ...
  host.append(node('p', 'eyebrow', `Colab companion · Week ${data.week}`), node('h3', '', data.title));
  host.append(node('p', 'lab-intro', 'Choose a notebook topic, compare input cases, then follow the Python code. One workspace shows what runs, what changes and what gets printed.'));
  ...
  workspace.append(codePane,statePane); host.append(workspace);
  const memoryPane = node('section','lab-memory-pane'); memoryPane.append(node('h4','','Collections in memory'));
  const memory = node('div','lab-memory'); memoryPane.append(memory); host.append(memoryPane);
  ...
  const legend = node('details','lab-help'); legend.append(node('summary','','How to read this animation'));
  ...
  function scopeBox(name, values, previous, top) {
    ...
    for (const [key,value] of Object.entries(values)) {
      ...
      if (value.ref) card.append(node('span','lab-ref-hint','↓ collection below'));
      list.append(card);
    }
  ...
  function drawMemory(frame, previous) {
    ...
    memory.replaceChildren(); memoryPane.hidden=!objects.size;
  ...
  timeline.addEventListener('input',()=>{stop();position=Number(timeline.value);paint();});
  back.addEventListener('click',()=>{stop();position=Math.max(0,position-1);paint();});
  next.addEventListener('click',()=>{stop();position=Math.min(selected.frames.length-1,position+1);paint();});
  reset.addEventListener('click',()=>{stop();position=0;paint();});
  play.addEventListener('click',()=>{if(timer){stop();return;}if(position===selected.frames.length-1)position=0;play.textContent='Pause';play.setAttribute('aria-pressed','true');paint();timer=setTimeout(advance,Number(speed.value));});
